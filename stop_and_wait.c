...
void A_output(struct msg message)
{
	...
	starttimer(A,time_oout);
	printf(">>>>>DATA DELIEVER TO B >>>>>>>>>>\n");
...
{
	int expchksum = packetA2B.checksum;
	int expack = packetA2B.seqnum;
	struct pkt rcva = packet;
	printf("\n>>>>>>>ARRIVE INPUT A>>>>>>>>>\n");

	if (packetA2B.checksum == rcva.checksum && packetA2B.seqnum == rcva.acknum) {
		stoptimer(A);
		printf("----->ACK#%d back to A", rcva.acknum);
	}
	else {
		printf("\n**************ACK#%d damage or loss***************\n", rcva.acknum);
	}

	//check the checksum see if the packet is broken
	//ckeck the ack see if the packet is expected
...
{
	printf(">>\n*********INTERRUPT!!packet#%d is retransmit to Layer 3 at host A************\n",packetA2B.seqnum);
	tolayer3(A, packetA2B);
	starttimer(A, time_oout);
...
{
	struct pkt rcvb = packet;
	struct msg tk;
	printf("\n>>>>>>>ARRIVE INPUT B >>>>>>>>>\n");
	if (seqnum == rcvb.seqnum && checksum == rcvb.checksum) {
		for (int i = 0; i < 20; i++)
		{
			tk.data[i] = rcvb.payload[i];
		}
		tolayer5(B, tk.data);

		ack_packet.seqnum = 0;
		ack_packet.acknum = rcvb.seqnum;
		ack_packet.checksum = rcvb.checksum;
		for (int i = 0; i < 20; i++)
		{
	...
	printf("ACK#%d has been sent", rcvb.seqnum);
...
void init();
void generate_next_arrival();
void printEnvptr(struct event *a);
...
void printEnvptr(struct event *a) {
	...
	if (c.evtype == 1) {
		if (c.eventity == 0) {
			printf("\n>>>>>next event type is OUTPUT A>>>>>\n");
		}
		else {
			printf("\n>>>>>next event type is OUTPUT B>>>>>\n");
		}	
	}
	else {
		if (c.eventity == 0) {
			printf("\n>>>>>next event type is INPUT A>>>>>\n");
		}
		else {
			printf("\n>>>>>next event type is INPUT B>>>>>\n");
		}
	}
}