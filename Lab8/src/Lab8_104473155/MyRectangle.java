...
public class MyRectangle {
	...
	public void processMousePressed(MouseEvent e) {
		...
		if (x-tor<e.getX() && e.getX() < x+tor && y-tor<e.getY() && e.getY()<y+height+tor || y-tor<e.getY() && e.getY() < y+tor && x-tor<e.getX() && e.getX()<x+width+tor) {
			color = Color.GREEN;
			resizing = false;
			
		}
		else if (x+width-tor<e.getX() && e.getX() < x+width+tor && y-tor<e.getY() && e.getY()<y+height+tor) {
			color = Color.BLUE;
			resizing = true;
			isX = true;
			
		}
		else if (y+height-tor<e.getY() && e.getY() < y+height+tor && x-tor<e.getX() && e.getX()<x+width+tor) {
			color = Color.BLUE;
			resizing = true;
			isX = false;
		}
	...
	public void processMouseDragged(MouseEvent e) {
		if (enable&&!resizing) {
			this.x = e.getX()-this.px + this.plx;
			this.y = e.getY() - this.py + this.ply;
			
		}
		else if(enable&&resizing&&isX) {
			this.width = e.getX() - this.px + this.plw;
			
		}
		else if(enable&&resizing&&!isX) {
			this.height = e.getY() - this.py + this.plh;
