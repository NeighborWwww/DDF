...
String toString2(int raw) {
  if (raw < 10) return "0" + String(raw);
...
//   lcd.setCursor(6,1);
//   lcd.print(n3);
     String toPrint = toString2(n1) + "." + toString2(n2) + "." + toString2(n3);
     lcd.print(toPrint);
//   Serial.print(n1);
//   Serial.print(".");
//   Serial.print(n2);
...
//  Serial.println(mode);
  Serial.println(toPrint+"   Mode is: "+mode);
}
...
ISR(TIMER1_COMPA_vect){ // This function runs once every time timer compare
  ...
  if (mode == 1 || mode ==2 || ((mode == 4)&&(counterFlg==0))){
    if(mode == 2){
      disp_d(set_buff1,set_buff2,set_buff3);
    }
    else{
